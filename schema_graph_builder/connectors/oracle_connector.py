"""
Oracle database connector with security enhancements
"""
...
def get_oracle_schema(config_path: str) -> Dict[str, Any]:
    """
    Extract schema from Oracle database with security enhancements.
    ...
    try:
        ...
        audit_logger.log_connection_attempt('oracle', host, database_identifier, username)
        
        # Create secure connection string
        db_url = ConnectionSecurity.create_secure_connection_string(
            'oracle', host, port, database_identifier, username, password,
        ...
        logger.info(f"Connecting to Oracle: {masked_url}")
        ...
        try:
            # Get all table names from the user schema
            ...
            audit_logger.log_connection_success('oracle', host, database_identifier)
            audit_logger.log_schema_extraction('oracle', database_identifier, len(schema['tables']))
            logger.info(f"Successfully extracted Oracle schema: {len(schema['tables'])} tables")
            
            return schema
            
        except Exception as e:
            error_msg = f"Failed to extract schema: {e}"
            audit_logger.log_connection_failure('oracle', host, database_identifier, error_msg)
            raise ConnectionError(error_msg) from e
            
    except Exception as e:
        if 'audit_logger' in locals() and host and database_identifier:
            audit_logger.log_connection_failure('oracle', host, database_identifier, str(e))
        raise
        ...
        if 'engine' in locals():
            try:
                engine.dispose()
                logger.debug("Oracle connection disposed")
            except Exception as e:
                logger.warning(f"Error disposing Oracle connection: {e}") 