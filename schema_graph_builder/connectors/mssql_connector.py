"""
MS SQL Server database connector with security enhancements
"""
...
def get_mssql_schema(config_path: str) -> Dict[str, Any]:
    """
    Extract schema from MS SQL Server database with security enhancements.
    ...
    try:
        ...
        audit_logger.log_connection_attempt('mssql', host, database, username)
        ...
        logger.info(f"Connecting to MS SQL Server: {masked_url}")
        ...
        try:
            ...
            audit_logger.log_connection_success('mssql', host, database)
            audit_logger.log_schema_extraction('mssql', database, len(schema['tables']))
            logger.info(f"Successfully extracted MSSQL schema: {len(schema['tables'])} tables")
            
            return schema
            
        except Exception as e:
            error_msg = f"Failed to extract schema: {e}"
            audit_logger.log_connection_failure('mssql', host, database, error_msg)
            raise ConnectionError(error_msg) from e
            
    except Exception as e:
        if 'audit_logger' in locals():
            audit_logger.log_connection_failure('mssql', host, database, str(e))
        raise
        ...
        if 'engine' in locals():
            try:
                engine.dispose()
                logger.debug("MSSQL connection disposed")
            except Exception as e:
                logger.warning(f"Error disposing MSSQL connection: {e}") 